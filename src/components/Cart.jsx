...
import { FaTimes, FaTrash } from 'react-icons/fa'; // Asegúrate de tener react-icons instalado

// Recibe la nueva prop onIncrementQuantity
const Cart = ({ cartItems, onRemoveFromCart, onCloseCart, isCartOpen, onCheckout, onIncrementQuantity, onDecrementQuantity }) => {
            ...
            cartItems.map(item => (
              <div key={item.id} className="flex items-center justify-between border-b last:border-b-0 py-4">
                <div className="flex items-center">
                  {/* La ruta de la imagen debe ser correcta */}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {/* Botón para añadir una unidad más del mismo producto */}
                  <button
                    onClick={() => onIncrementQuantity(item)}
                    className="text-green-600 text-2xl hover:text-green-700 transition duration-300"
                  >
                    +
                  </button>
                </div>
